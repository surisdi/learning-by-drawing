...
def run_command(args):
    ...
    cachedir = os.path.join(args.outdir, safe_dir_name(layer), 'causal',
            classname)
...
def ace_loss(segmenter, classnum, model, layer, high_replacement, ablation,
    ...
    model.replacement = None
    tensor_images = model(pbatch.cuda())
    assert model.ablation[layer] is a_p_mask
    erase_effect = segmenter.predict_single_class(
            tensor_images, classnum)[0]
    ...
    if run_backward:
        erase_loss.backward()
    # Second, put in max for the selected units.  Loss is amount
    # of missing object.
    ...
    model.replacement = high_replacement
    tensor_images = model(cbatch.cuda())
    assert model.ablation[layer] is a_c_mask
    add_effect = -segmenter.predict_single_class(
            tensor_images, classnum)[0]
